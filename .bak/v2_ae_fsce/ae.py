"""v2_ae_fsce：在 v2_ae 的架構跟 Poisson NLL 之外，額外加一個 FSCE（fuzzy set
cross entropy，UMAP 訓練用的那個 loss）項，實測「用 UMAP 的 loss 把高維 kNN
鄰接關係直接拉進 latent 距離」對這條沒有 tanh 夾住 latent 的基準線有什麼影響。

跟 v2_ae_tanh_fsce 唯一的差異只在 encoder：這裡沿用 v2_ae 的設計——latent
前不接任何激活，離群值不會被壓回來。FSCE 對「該分開的點」持續施加推力，
在不受 tanh 邊界限制的空間裡，理論上更容易做到「latent 距離跟高維鄰接關係
對齊」，不用像 v2_ae_tanh_fsce 那樣煩惱 UMAP 的 a/b 校準跟 (-1,1)² 盒子打架。

FSCE 的高維鄰接關係是用「整包 count 向量的 log1p、cosine 距離」kNN 建的
fuzzy simplicial set（跟 data/patch/umap_grid.py 的可行性檢查用同一種 metric），
只建一次、整個訓練共用同一張圖，不是每個 epoch 重建。
"""
...
class MLPAE(nn.Module):
    """跟 v2_ae 的 MLPAE 完全同架構，唯一差異是多一個 encode()，讓 FSCE loss
    可以只跑 encoder、不用連 decoder 一起算。
    """
