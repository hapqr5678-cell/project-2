"""畫出 v0_sizefactor 的 latent space，並檢查它到底編碼了什麼。

這一版的驗收標準跟其他版本不一樣。其他版本是「希望 R²密度 低一點」，
這裡是「R²密度 應該要塌下來」——密度已經被 size factor 從 encoder 的輸入
裡除掉了，如果 latent 還能還原 log(POI 數)，代表正規化沒生效，是 bug。

所以多印兩個組成類的量，看密度讓出來的空間被什麼填滿：
  最大類佔比   單一類別獨大的程度（車站周邊 vs 商店街）
  組成熵       類別分布有多平均，越高越混合
如果這兩個的 R² 明顯比 v0_poisson_nll 高，就是這一版真正的收穫。

「過飽和」的判定改到 analyze/saturation.py，這裡的 robust 距離離群
只代表「形狀怪」，不再等於「密度異常」。
"""

import os
import sys

import numpy as np
import matplotlib as mpl
import matplotlib.pyplot as plt
from scipy.spatial import cKDTree

sys.path.insert(0, os.path.abspath(f"{os.path.dirname(__file__)}/../../.."))
from config.dataset import N_CAT, PATCHES, RADIUS, result  # noqa: E402

LATENTS = result("v0_sizefactor", "latents.npz")
OUT = result("v0_sizefactor", "latent_plot.png")

ZOOM_PCT = (1, 99)      # 放大視角的座標範圍百分位
OUTLIER_PCT = 99.5      # robust 距離超過這個百分位的點算離群
KNN = 50
DOT = 3.0

mpl.rcParams["font.family"] = ["Heiti TC"]
mpl.rcParams["axes.unicode_minus"] = False
mpl.rcParams["figure.dpi"] = 130


def patch_stats(p):
    """一次掃過 patches，回傳 (偏心度, 最大類佔比, 組成熵)。"""
    offs, dx, dy, cat = p["offsets"], p["dx"], p["dy"], p["cat"]
    n = len(offs) - 1
    ecc = np.zeros(n)
    top = np.zeros(n)
    ent = np.zeros(n)
    for i in range(n):
        s, e = offs[i], offs[i + 1]
        ecc[i] = np.hypot(dx[s:e].mean(), dy[s:e].mean()) / RADIUS
        frac = np.bincount(cat[s:e], minlength=N_CAT).astype(float)
        frac /= frac.sum()
        top[i] = frac.max()
        nz = frac[frac > 0]
        ent[i] = -(nz * np.log(nz)).sum()
    return ecc, top, ent


def knn_r2(z, y):
    """用 latent 的 k 個鄰居預測 y，回傳 R²（排除自己）。"""
    zs = (z - z.mean(0)) / z.std(0)
    _, idx = cKDTree(zs).query(zs, k=KNN + 1)
    pred = y[idx[:, 1:]].mean(1)
    return 1 - np.var(y - pred) / np.var(y)


def robust_distance(z):
    """對每個軸用 中位數 / MAD 標準化後取歐氏距離，不被離群值本身帶偏。"""
    med = np.median(z, axis=0)
    mad = np.median(np.abs(z - med), axis=0) * 1.4826
    return np.linalg.norm((z - med) / mad, axis=1)


def scatter(ax, z, c, title, label, cmap="viridis"):
    sc = ax.scatter(z[:, 0], z[:, 1], c=c, s=DOT, cmap=cmap, linewidths=0,
                    alpha=0.6, rasterized=True,
                    vmin=np.percentile(c, 1), vmax=np.percentile(c, 99))
    cb = ax.figure.colorbar(sc, ax=ax, fraction=0.046, pad=0.02)
    cb.set_label(label, fontsize=8)
    cb.ax.tick_params(labelsize=7)
    style(ax, title)


def style(ax, title):
    ax.set_title(title, fontsize=10)
    ax.set_xlabel("z1", fontsize=8)
    ax.set_ylabel("z2", fontsize=8)
    ax.tick_params(labelsize=7)
    ax.grid(alpha=0.15, linewidth=0.5)
    for s in ax.spines.values():
        s.set_alpha(0.3)


def zoom(ax, z):
    lo = np.percentile(z, ZOOM_PCT[0], axis=0)
    hi = np.percentile(z, ZOOM_PCT[1], axis=0)
    pad = (hi - lo) * 0.05
    ax.set_xlim(lo[0] - pad[0], hi[0] + pad[0])
    ax.set_ylim(lo[1] - pad[1], hi[1] + pad[1])


def main():
    d = np.load(LATENTS)
    z, n_poi, err = d["z"], d["n_poi"], d["err"]
    lat, lon = d["lat"], d["lon"]
    ecc, top, ent = patch_stats(np.load(PATCHES))

    print(f"latent 能還原多少（kNN R², k={KNN}）")
    for name, y in [("log(POI 數) ← 該塌掉", np.log(n_poi.astype(float))),
                    ("偏心度", ecc),
                    ("最大類佔比", top),
                    ("組成熵", ent),
                    ("Poisson deviance", err.astype(float))]:
        print(f"  {name:<22}R² = {knn_r2(z, y):+.3f}")

    dist = robust_distance(z)
    out = dist > np.percentile(dist, OUTLIER_PCT)
    print()
    print(f"形狀離群 {out.sum()} 個：POI 數中位數 {np.median(n_poi[out]):.0f} "
          f"(全體 {np.median(n_poi):.0f})，"
          f"最大類佔比 {np.median(top[out]):.3f} (全體 {np.median(top):.3f})")

    fig, axes = plt.subplots(2, 3, figsize=(15, 9))
    (a, b, c), (e, f, g) = axes

    scatter(a, z, np.log10(n_poi), "全域：色=POI 數（應該看不出結構）",
            "log10(POI 數)")
    zoom(b, z)
    scatter(b, z, top, f"放大 {ZOOM_PCT[0]}~{ZOOM_PCT[1]}%：色=最大類佔比",
            "max 類別佔比", cmap="plasma")
    zoom(c, z)
    scatter(c, z, ecc, "放大：色=偏心度", "|質心|/R", cmap="magma")

    zoom(e, z)
    scatter(e, z, err, "放大：色=重建誤差", "Poisson deviance")

    f.scatter(z[~out, 0], z[~out, 1], s=DOT, c="#b8bcc4",
              linewidths=0, alpha=0.5, rasterized=True)
    f.scatter(z[out, 0], z[out, 1], s=DOT * 4, c="#c0392b",
              linewidths=0, alpha=0.8, rasterized=True,
              label=f"形狀離群 (>{OUTLIER_PCT}%，n={out.sum()})")
    f.legend(fontsize=8, markerscale=2, framealpha=0.9)
    style(f, "全域：robust 距離離群點")

    g.scatter(lon[~out], lat[~out], s=1.5, c="#b8bcc4",
              linewidths=0, alpha=0.4, rasterized=True)
    g.scatter(lon[out], lat[out], s=8, c="#c0392b",
              linewidths=0, alpha=0.85, rasterized=True, label="形狀離群 patch")
    g.legend(fontsize=8, markerscale=2, framealpha=0.9)
    g.set_aspect(1 / np.cos(np.deg2rad(float(lat.mean()))))
    g.set_title("形狀離群 patch 的地理位置", fontsize=10)
    g.set_xlabel("經度", fontsize=8)
    g.set_ylabel("緯度", fontsize=8)
    g.tick_params(labelsize=7)
    g.grid(alpha=0.15, linewidth=0.5)

    fig.suptitle(f"v0_sizefactor latent space（{len(z)} 個 patch，latent_dim=2，"
                 f"size factor 正規化 + 圓內 Poisson NLL）", fontsize=12)
    fig.tight_layout()
    fig.savefig(OUT, bbox_inches="tight")
    print(f"\n已存 {OUT}")


main()
