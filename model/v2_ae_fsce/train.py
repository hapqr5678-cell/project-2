"""訓練 v2_ae_fsce：跟 v2_ae 的 train.py 相比，多了兩件事——訓練前先在高維
count 向量上建一次 FSCE 用的 fuzzy graph，訓練迴圈裡每個 step 除了原本的
reconstruction batch，額外抽一批正樣本邊（來自 fuzzy graph）跟等量的隨機
負樣本邊，把 FSCE loss 加權後跟 Poisson NLL 加在一起、共用同一次
backward/opt.step()。細節同 v2_ae_tanh_fsce，差別只在底下的模型沒有 tanh。
