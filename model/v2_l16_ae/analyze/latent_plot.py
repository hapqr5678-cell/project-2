"""畫出 v2_ae 的 latent space，並檢查它到底編碼了什麼。

v2_ae 是 v2 系列（v2_ae / v2_perceiver / v2_vae）的基準組，這裡的 R²、
離群點分布是後兩者共同的對照基準：如果 v2_perceiver / v2_vae 的 kNN R²
明顯比這裡低，代表換 encoder 架構確實把 latent 從密度手上搶回來了一些。

「偏心度」這欄特別值得看：v2 系列的輸入根本沒有 x,y 座標（只是整包
...
def main():
    ...
    if z.shape[1] > 2:
        print(f"\n將 {z.shape[1]} 維 latent 降維到 2 維 (PCA)...")
        mean = z.mean(0)
    ...
    fig.suptitle(f"{VERSION} latent space（{len(z)} 個 patch，latent_dim={z.shape[1]}，"
                 f"整包類別 count 向量 + Poisson NLL，MLP encoder 基準組）", fontsize=12)
    fig.tight_layout()
