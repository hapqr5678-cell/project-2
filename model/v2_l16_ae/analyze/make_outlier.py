...
v2_ae 是基準組，這支圖是 v2_perceiver / v2_vae 同一支腳本比較的對象：
換了 encoder 架構後，同樣「加某一類 POI」的動作會不會讓 latent
移動得更乾脆、離群門檻跨越得更早或更晚。
...
from ae import MLPAE, N_CAT, poisson_deviance  # noqa: E402
...
def background(ax, zp_all):
    ax.scatter(zp_all[:, 0], zp_all[:, 1], s=DOT, c="#b8bcc4",
...
def main():
    ...
    if z_all.shape[1] > 2:
        print(f"將 {z_all.shape[1]} 維 latent 降維到 2 維 (PCA)...")
        mean = z_all.mean(0)
        _, _, vt = np.linalg.svd(z_all - mean, full_matrices=False)
        to2d = lambda z: (z - mean) @ vt[:2].T
    else:
        to2d = lambda z: z
    zp_all = to2d(z_all)
    ...
    for c in range(N_CAT):
        ...
        print(f"  {CAT_ZH[c]:<6} 加 {ADD_STEPS[-1]} 個後 "
              f"z=({', '.join(f'{v:+6.2f}' for v in z[-1][:4])}{'...' if LATENT_DIM>4 else ''})  "
              f"robust 距離 {dist[0]:.2f} -> {dist[-1]:.2f}  "
              + (f"{cross[0]} 個時越過門檻" if cross else "沒越過門檻"))

    traj = to2d(trajs[ADD_CAT])
