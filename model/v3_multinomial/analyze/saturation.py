...
為什麼要換一套判定：在 v2_ae 上，「latent 離群」同時混著兩種東西——
POI 特別多、以及類別組合特別怪——因為密度本身幾乎佔滿了 latent
（v2_ae 的 latent→log(POI 數) kNN R² = 0.972）。分不出來的話，
「過飽和」這個結論就沒有內容，它可能只是在說「這裡 POI 很多」，
那不用 autoencoder 也知道。

v3_multinomial 把密度從 latent 拿掉之後（同一個 R² 掉到 0.174），
兩者才分得開：
    z            = 類別組成，跟總量無關
...
已知風險：
  * 鄰居是在 2 維 latent 上找的，「組成像不像」的定義完全由模型決定。
    latent 若塌成一條線，鄰居就沒有意義，要先看 latent_plot 的散布。
  * k=50 在 9770 個 patch 上約佔 0.5%，但 latent 邊緣的點鄰居會被拉得很遠，
    那裡的 z-score 不可信。所以圖上另外標出「鄰居平均距離」最大的那批。
  * patch 中心是規則格點（CENTER_STEP=100m、HALF_WIDTH=50m，剛好不重疊，
    但相鄰 patch 的街廓仍然高度相關），kNN 很可能挑到地理上的鄰居而不是
    組成上的同類。這會讓 z-score 偏保守（自己跟自己比）。要根治得在取鄰居時
    排除地理距離太近的，這一版沒做。
  * v3 每個 patch 的 POI 數中位數只有 20，類別比例本身就是 20 次抽樣的結果，
    噪聲很大。latent 上的「組成鄰居」有一部分其實是抽樣噪聲的鄰居，
    這是目前 z-score 最主要的不確定來源。
"""
...
LATENTS = result("v3_multinomial", "latents.npz")
OUT = result("v3_multinomial", "saturation.png")
...
def main():
    ...
    fig.suptitle("v3_multinomial 飽和度：latent 只管形狀，密度單獨比較\n"
