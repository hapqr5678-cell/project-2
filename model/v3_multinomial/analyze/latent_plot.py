"""畫出 v3_multinomial 的 latent space，並檢查它到底編碼了什麼。

這一版最該盯的是「log(POI 數) ← 密度」那一行的 kNN R²。
v2_ae 在同一份資料上是 +0.972——latent 幾乎完全被密度佔滿；
v3 把總量改成 decoder 的 offset 之後掉到 +0.174。這個數字要是又爬回去，
代表 multinomial 分解沒生效（最可能的原因是 encoder 又從輸入拿到了
總量資訊，檢查 ae.py 的 to_composition()）。

注意這一版的 robust distance 離群點不再等於「過飽和」——latent 不含密度，
距離只代表「類別組合怪」。飽和度判定看 analyze/saturation.py。

「偏心度」這欄特別值得看：v2/v3 系列的輸入根本沒有 x,y 座標（只是整包
類別 count 向量），模型從沒看過哪個 POI 在圓的哪一側。如果 latent
對偏心度的 kNN R² 仍然明顯大於 0，那不是模型記得座標，而是「類別組成
剛好跟偏心度相關」的巧合（例如某類 POI 常見於道路旁、道路又常貼著
patch 邊緣），這件事本身也值得在報告裡說清楚。
"""
...
def main():
    ...
    fig.suptitle(f"{VERSION} latent space（{len(z)} 個 patch，latent_dim=2，"
                 f"類別組成比例 + log n offset，密度已移出 latent）", fontsize=12)
    fig.tight_layout()
