...
長條圖，用 --n 指定 patch 編號（0 起算）。跟 v2 各版的 rebuild.py 同一種圖，
差別是 encode 要多餵一份該格的 OD 矩陣，並且多印 alpha 與這格的 OD 量。
"""
...
sys.path.insert(0, os.path.abspath(f"{os.path.dirname(__file__)}/../../.."))
from ae import (GATLiteralAE, ODGraph, Patches, poisson_deviance,  # noqa: E402
                poisson_nll)
...
def main():
    ...
    x = data.agg(idx)
    with torch.no_grad():
        z, log_lam, _ = model(x, od.A_local[idx])
    cnt = x[0].numpy()
    ...
    print(f"latent z = ({z[0, 0]:+.3f}, {z[0, 1]:+.3f})   "
          f"alpha = {model.alpha().item():.4f}（全模型共用一個）")
    print(f"OD 量 = {od.n_od[n].item():.0f}（全體中位數 "
          f"{od.n_od.median().item():.0f}，最大 {od.n_od.max().item():.0f}）")
    ...
    ax.set_title(f"{VERSION} patch {n}（deviance {dev:.4f}，"
                 f"POI {int(data.n_poi[n])} 個，OD {od.n_od[n].item():.0f} 筆）",
                 fontsize=11)
