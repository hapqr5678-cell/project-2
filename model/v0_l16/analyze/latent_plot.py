...
def main():
    ...
    zp, var_ratio = pca2(z)          # 只給畫圖用
    print()
    print(f"PCA 投影：PC1 {var_ratio[0] * 100:.1f}%、"
          f"PC2 {var_ratio[1] * 100:.1f}% 的變異（共 {z.shape[1]} 維）")
    ...
    style(f, "全域：robust 距離離群點（距離用完整 16 維算）")
